...
<h1>Modification d'un Joueur</h1><br>
 ...
 <form method="post" action="{{route('Joueurs.update', $Un_Joueur->pluck('idPersonne')->first())}}">
@csrf
@method("put")
    <div class="row">
        <div class="col-md-5">
            <div class="form-group row">
                ...
                <input type="text" class="form-control col-sm-6" name="nomPersonne" value="{{$Un_Joueur->pluck('nomPersonne')->first()}}" required>
            </div>

            <div class="form-group row">
                <label for="date" class="col-sm-6"><h4>Prenom</h4></label>
                <input type="text" name="prenomPersonne" value="{{$Un_Joueur->pluck('prenomPersonne')->first()}}">
            </div>
